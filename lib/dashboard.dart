...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void _startMeasuringSession(int seconds) {
    // Pastikan timer lama benar-benar dihentikan
    _countdownTimer?.cancel();

    // Pastikan animasi kembali ke awal
    _measuringAnimationController.stop();
    _measuringAnimationController.reset();

    // Atur durasi sesuai pilihan pengguna
    ...
    // ==========================================================
    // ANIMASI BERJALAN TERUS DARI 0% → 100%
    // ==========================================================
  ...
  void _finishAndSaveSession() {
    ...
    if (sessionMoisture.isNotEmpty && sessionTemp.isNotEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Pengukuran $recordIndex dari "${plot.name}" tersimpan!',
          ),
                              ...
                              child: Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Flexible(
                                      child: Text(
                                        'Pastikan HP tersambung ke WiFi '
                                        '"CitriSoil_ESP32" untuk membaca data tanah.',
                                        style: TextStyle(
                                          fontSize: 11.5,
                                          fontWeight: FontWeight.w600,
                                          color: Colors.amber.shade900,
                              ...
                              child: Stack(
                                children: [
                                  if (hasImage) ...[
                                    Positioned.fill(
                                      child:
                                          currentPlot.imagePath!.startsWith(
                                            'assets/',
                                          )
                                          ? Image.asset(
                                              currentPlot.imagePath!,
                                              fit: BoxFit.cover,
                                            )
                                          : Image.file(
                                              File(currentPlot.imagePath!),
                                              fit: BoxFit.cover,
                                            ),
                                    ),
                                    Positioned.fill(
